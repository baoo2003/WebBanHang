										...
										<div class="notification-item" onclick="submitForm('formNotifi-${notification.id}')">
											<form id="formNotifi-${notification.id}" action="updateNotifi.htm" method="POST">
												...
												<a class="notification-link">
													...
													<c:if test="${not notification.status}">
														<span class="status-dot" style="height: 10px; width: 10px; background-color: ${notification.status ? '#fff' : '#81C408'}; border-radius: 50%; display: inline-block;flex-shrink:0;"></span>
													</c:if>
												</a>
											</form>
										</div>
							...
							<c:when test="${not empty carts}">
								<c:forEach var="cart" items="${carts}">
									<tr data-product-id="${cart.cartId.productId}"
										...
										<td>
											<form method="post" action="loadCart.htm"
												class="quantity-form">
												<div class="input-group quantity mt-4" style="width: 100px;">
													<div class="input-group-btn">
														<input type="hidden" name="customerId"
															value="${cart.customer}" /> <input type="hidden"
															name="productId" value="${cart.product}" />
														<button type="submit"
